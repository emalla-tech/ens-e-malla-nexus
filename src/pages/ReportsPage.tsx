...
export function ReportsPage() {
  ...
  const hasCashData = monthly.some((item) => item.income > 0 || item.expense > 0);
  ...
  return <div className="report-page space-y-6 print:bg-white">
    <section className="hidden border-b-2 border-brand-orange pb-3 print:flex print:items-end print:justify-between"><div><p className="text-2xl font-black"><span className="text-brand-orange">E</span>N<span className="text-brand-orange">s</span></p><p className="text-xs font-bold uppercase tracking-[0.18em] text-gray-500">E-Malla Nexus</p></div><div className="text-right"><p className="font-black">Executive Performance Report</p><p className="text-xs text-gray-500">Generated {today.toLocaleDateString('en', { day: 'numeric', month: 'long', year: 'numeric' })} · {period === 365 ? 'Last 12 months' : `Last ${period} days`}</p></div></section>
    ...
    <section className="grid gap-6 xl:grid-cols-[1.35fr_0.65fr]">
      <article className="report-chart rounded-md border border-gray-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between"><div><h2 className="text-lg font-black">Cash-flow trend</h2><p className="mt-1 text-sm text-gray-500">Income and expenses by month</p></div><span className={`rounded-md px-2 py-1 text-xs font-bold ${income >= expenses ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>{income >= expenses ? 'Positive' : 'Negative'} cash flow</span></div>{hasCashData ? <><div className="cash-bars mt-6 flex h-56 items-end gap-3 border-b border-gray-200 px-2">{monthly.map((item) => <div key={item.key} className="flex min-w-0 flex-1 flex-col items-center"><div className="bar-area flex h-44 w-full items-end justify-center gap-1"><div title={`Income ${money.format(item.income)}`} style={{ height: `${Math.max(item.income ? 4 : 0, item.income / maxMonthly * 100)}%` }} className="w-2/5 rounded-t bg-emerald-500" /><div title={`Expenses ${money.format(item.expense)}`} style={{ height: `${Math.max(item.expense ? 4 : 0, item.expense / maxMonthly * 100)}%` }} className="w-2/5 rounded-t bg-red-400" /></div><p className="mt-2 truncate text-xs font-semibold text-gray-500">{item.label}</p></div>)}</div><div className="mt-4 flex gap-5 text-xs font-semibold text-gray-500"><span className="flex items-center gap-2"><i className="h-2.5 w-2.5 rounded-sm bg-emerald-500" />Income</span><span className="flex items-center gap-2"><i className="h-2.5 w-2.5 rounded-sm bg-red-400" />Expenses</span></div></> : <div className="mt-5 rounded-md bg-gray-50 p-5 text-center text-sm text-gray-500">No income or expense transactions were recorded during this period.</div>}</article>
